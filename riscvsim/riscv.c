...
bool execute(struct address_space *mem, struct cpu *cpu, int32_t inst) {
	...
	break;
    case 0x33:
	okay = execute_alu(cpu, get_rd(inst), get_rs1(inst), get_rs2(inst), get_funct3(inst), get_funct7(inst));
	cpu->pc += 4;
	break;
    case 0x63:
...
bool execute_alu_imm(struct cpu *cpu, int8_t funct3, int8_t rd, int8_t rs1, int32_t imm) {
	...
	sam = imm & 0x1f;
	uint32_t fu7 = imm & 0xfe0;	
	if (fu7 == 0) {
	    cpu->x[rd] = cpu->x[rs1] << sam;
	    printf("%s <- %d\n", r[rd], cpu->x[rd]);
	} else
	    return false;
	break;
	...
	val = cpu->x[rs1];
	if (f7u == 0) {
	    printf32("srli    %s, %s, %d", r[rd], r[rs1], sam);
	    cpu->x[rd] = val >> sam;
	    printf("%s <- %d\n", r[rd], cpu->x[rd]);
	} else {
	    int32_t sam = imm & 0x1f;
	    int32_t val = cpu->x[rs1];
	    printf32("srai    %s, %s, %d", r[rd], r[rs1], sam);
	    cpu->x[rd] = val >> sam;
	    printf("%s <- %d\n", r[rd], cpu->x[rd]);
	} break;
    case 6:
...
bool execute_alu(struct cpu *cpu, int8_t rd, int8_t rs1, int8_t rs2, int8_t funct3, int8_t funct7) {
    switch(funct3) {
    case 0:
        if(funct7 == 0) {
	...
	if(funct7 == 1) {
	    ...
	    int64_t big = (int64_t)cpu->x[rs1] * (int64_t)cpu->x[rs2];
	    cpu->x[rd] = big >> 32;
	    printf("%s <- %d\n", r[rd], cpu->x[rd]);
	}
	else {
	...
	if(funct7 == 1) {
	    ...
	    uint64_t big = (int64_t)cpu->x[rs1] * (uint64_t)cpu->x[rs2];
	    cpu->x[rd] = big >> 32;
	    printf("%s <- %d\n", r[rd], cpu->x[rd]);
	}
	else {
	...
	if(funct7 == 1) {
	    ...
	    uint64_t big = (uint64_t)cpu->x[rs1] * (uint64_t)cpu->x[rs2];
	    cpu->x[rd] = big >> 32;
	    printf("%s <- %d\n", r[rd], cpu->x[rd]); 
	}
	else {
	...
	if(funct7 == 0) {
	    ...
	    cpu->x[rd] = (uint32_t)cpu->x[rs1] >> (cpu->x[rs2] & 0x1f);
	    printf("%s <- %d\n", r[rd], cpu->x[rd]);
	    break;
	}
	else if(funct7 == 0x20) {
...
bool execute_load(struct address_space *mem, struct cpu *cpu, int8_t funct3, int8_t rd, int8_t rs1, int32_t imm) {
	...
	cpu->x[rd] = (uint32_t)lb(mem, ((uint32_t)cpu->x[rs1] + (uint32_t)imm));
	printf("%s <- %u [%u]\n", r[rd], cpu->x[rd], (cpu->x[rs1] + imm));
	break;
    case 5:
	if(imm == 0)
	...
	cpu->x[rd] = (uint32_t)lh(mem, ((uint32_t)cpu->x[rs1] + (uint32_t)imm));
	printf("%s <- %u [%u]\n", r[rd], cpu->x[rd], (cpu->x[rs1] + imm));
	break;
    }
    return true;
...
bool execute_store(struct address_space *mem, struct cpu *cpu, int8_t funct3, int8_t rs1, int8_t rs2, int32_t imm) {
	...
	sw(mem, (cpu->x[rs1] + imm), cpu->x[rs2]);
	printf("[%d] <- %d\n", (cpu->x[rs1] + imm), cpu->x[rs2]);
	break;
    }
    return true;
}
